...
CA.create = function (options, callback) {
  var caFolder = options.caFolder;
  var caAttrs = options.caAttrs || CAattrs;
    ...
    function (callback) {
      FS.exists(path.join(ca.certsFolder, 'ca.pem'), function (exists) {
        if (exists) {
          ...
          ca.generateCA(caAttrs, callback);
        }
      });
    }
  ], function (err) {
...
CA.prototype.generateCA = function (caAttrs, callback) {
  var self = this;
...
CA.prototype.generateServerCertificateKeys = function (hosts, serverAttrs, cb) {
  serverAttrs = serverAttrs || ServerAttrs;
  var self = this;
